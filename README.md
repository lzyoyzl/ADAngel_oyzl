...
[A100 O3 优化记录](docs/a100_o3_optimization.md)。production切换后的独立24样本/3轮
配对达到当前O1的2.022×（95% CI [2.013,2.025]），输出逐位一致、MSE不变。
正式入口四模式与内存安全复验已完成。四模式独立运行的compute-only约1.998×，
cold/steady约1.794×/1.828×；主门槛通过但超过2×的余量很小，不保证每种运行状态均超过2×。
5090 O1/O3的magic-bias候选在同步修复后的24样本/3轮配对中，速度比分别0.918×/0.993×，
MSE不变，均未加速，按用户要求停止后续magic实验、不晋升。共享scale同步风险已在v7修复：每个writer直接
acquire可复用stage、写后release；5090已重编译，合成集及真实4096³的racecheck/memcheck、
216项synccheck、129项集成测试和完整指令审计通过。24样本输出逐位一致、MSE不变。
production保留普通INT32转换，但已包含同步修复；不是继续使用原有不安全二进制。
四模式单样本冒烟通过；v8正式主配对360条完整，四模式主动停止时只有354/480条，
保留部分数据但不作为完整四模式结论，不再补跑或追加NCU。所有CV超标项仍保留并披露。
