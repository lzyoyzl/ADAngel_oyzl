...
最终提交同步后执行了完整 O0–O4、24样本交错复跑。480条记录全部有效且
`CV<3%`；O1/O3 compute-only跨样本 median 为 `0.623776/2.065672 ms`，逐样本
O3/O1等效吞吐比 median 为 `30.526%`。这次长时间未锁频运行与 O3单独复跑的
`2.015720 ms` 相差约2.5%，但两者均稳定地落在 O1吞吐约30%–31%的区间，不影响
“当前严格语义与工具链下无法达到50%”的结论。
