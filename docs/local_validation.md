...
```text
21 项纯 Python 单元测试通过（含 O0/O1 正式后端静态契约）
...
6. 运行 `python scripts/validate_o0.py`、`python scripts/validate_o1.py` 和对应原生集成
   测试，确认 O0 HMMA 与 O1 IMMA 算法 flags、转换、FP32 输出和四种计时模式全部通过。
7. 用 `4096^3` 运行 `validate_o1.py`，归档 `reports/o1_4096_validation.json`。
