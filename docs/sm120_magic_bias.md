...
最终决定：同步修复后的v8正式配对仍未发现magic加速，按用户要求停止后续实验。
O1/O3继续使用已修复同步的普通转换production，保留magic内部候选和全部原始证据。
v8完整主配对为360条；四模式在用户要求停止时中断，只保留354条部分记录，不作为完整结果。
...
修复后的完整主配对见下文v8；四模式正式运行按用户要求中断，不再补跑。
...
## 同步修复后的最终主配对v8：不采用magic

`runs/sm120_magic_paired_v8`完整包含24样本×5实现×3轮=360条记录。
warmup50、repeats200、conversion inner100；同进程交错顺序，不锁频、不等待GPU空闲，
不剔除离群点。二进制与v7安全验收一致，CUDA源码没有再改动。

| 实现 | 24样本GEMM median ms | 配对T普通/Tmagic中位数 | 配对bootstrap 95% CI |
|---|---:|---:|---|
| O0参考 | 0.767568 | — | — |
| O1普通转换 | 0.620656 | 1 | — |
| O1 magic | 0.660576 | 0.917817 | [0.906463, 0.930132] |
| O3普通转换 | 2.062592 | 1 | — |
| O3 magic | 2.075200 | 0.993124 | [0.992942, 0.993729] |

配对速度比先按同样本、同轮计算，再按轮和样本取中位数；延迟列则使用各样本合并轮次的
中位数。因此速度比不是简单用表中两个全局延迟相除。两项CI均低于1，各24个样本的
配对速度比也都低于1，没有充分理由晋升magic。

| 后端（普通与magic输出逐位一致） | MSE median vs O0 | MSE mean vs O0 |
|---|---:|---:|
| O1 | 9.823381579891711e-9 | 1.1141469767717465e-8 |
| O3 | 0.006653010195220884 | 0.007578844400053298 |

原始逐轮共有76个CV>=3%的stage测量条目；跨轮合并统计有40个超标stage条目，两者口径不同。
全部保留，不把这些记录改标成CV通过。解锁频率和共享运行环境可能引起波动，但没有证据
将每个离群点归因于特定进程。上述结论限于本次24样本配对，不宣称在所有输入/运行环境中
magic必然更慢，也不将bootstrap区间解读为对所有GPU调度状态的保证。

同步修复不改变partial值域或magic的精确性；其仍是将I2FP换成IADD+FADD，
不是无代价删除转换。v7同函数SASS审计确认指令替换、MMA/TMA/FMA与零spill保持。
v4 NCU只能作历史机制观察，不能当成修复后NCU数据；用户要求停止后没有新增v8 NCU。

### 用户要求停止的范围

`runs/sm120_magic_four_modes_v8`在主配对已确认无加速后，由用户要求停止：
共354条完整JSON记录、涉及18个样本；最后停在`layer_24_k_proj`的cold模式。
完整四模式应为480条，当前不完整，不生成或引用完整四模式汇总，也不只取已完成部分
冒充24样本结论。`four_modes_v8.log`保留KeyboardInterrupt；这是主动停止，不是CUDA故障。
后续NCU与进一步magic优化不再执行，先前v7四模式冒烟仅保留其冒烟结论。

v8原始主配对、部分四模式及日志已归档：`tmp/sm120_magic_v8_evidence.tar.gz`，SHA-256：
`644304ad4c70b2af82a31ea3d26eb5626628c3c7661d0eb270e1fd933a1a4a50`。
结果核验脚本`verify_sm120_magic_results.py --paired-only`只认证主配对完整性，不认证四模式；
会检查唯一记录、24样本身份、200次正有限计时、计时口径、同步元数据、MSE一致性以及与安全
验收相同的二进制/源码哈希。核验不删除或放宽标记CV超标项。
本地核验已通过，输出为`reports/sm120_magic/verified_paired_v8.json`；归档下载后的
SHA-256也与服务器一致。
