...
状态：目标一主验收完成，production已通过独立24样本配对门槛、四模式、数值与安全验证。
性能约为O1的2倍，主门槛通过但余量较小，不保证所有测量状态都超过2倍。
下一步目标二明确覆盖5090 O1和O3的magic-bias候选；A100结果不能替代SM120实测。
...
第二十四轮（24样本/3轮/warmup50/repeats200）：

| O3候选 | GEMM样本中位数 ms | 配对当前O1加速比 | bootstrap 95%区间 |
|---|---:|---:|---|
| 独立partial/K256 | 0.576000 | 1.98900 | [1.97902, 1.99461] |
| 仅scale2 | 0.575488 | 1.99734 | [1.99110, 2.00887] |
| 仅store2 | 0.568832 | 2.02613 | [2.01457, 2.03473] |
| store2+scale2 | 0.573952 | 2.00442 | [1.99279, 2.01243] |

同轮当前O1为1.145856ms。仅store2的24个样本中22个配对比值>2，范围
[1.98399,2.09109]；72条逐轮GEMM记录CV中位数1.08097%，最大6.50319%，
其中11条>=3%。所有候选全部阶段的跨轮汇总共有98条CV异常，全部保留。
该轮支持store2的总体配对中位数超过2，不宣称每个样本都超过2或所有计时均无波动。

240项GPU逐位检查、76项单元测试通过；全部24样本新旧O3输出逐位一致。
所有候选MSE median=0.006653010285119311、mean=0.00757884701115429；
当前O1 MSE median=9.82338825329489e-9、mean=1.1142038139865581e-8，均与此前一致。
store2正式函数包含原生U4/S4 IMMA和LDGSTS，16条静态STG.E.64；快速路径
REG128/STACK40，保留spill警告。scale2版本确认2条静态LDG.E.U16，但不采用，
因为未提供更好的整体收益。O1独立指令审计通过。

选中store2另完成60项memcheck、60项racecheck，以及完整4096³各一次：全部
0 errors/0 hazards。四模式已完成，见下节；仍需production切换后独立复验，
目标一尚未正式关闭，目标二仍未开始。

## Store2候选的完整四模式结果

来源`runs/a100_o3_four_modes_v24`：24样本×4实现（包括保留的旧O3 baseline）×4模式，
共384条唯一完整记录，每阶段200次计时，warmup50、conversion inner100；下表只列
当前O0、当前O1和选中的store2 O3，单位ms，数值为24个样本延迟中位数的中位数。
这是候选二进制结果，不冒充production切换后的复验。所有输出/MSE检查通过；
共201条阶段统计CV>=3%，未删除或挑选样本。此前功率诊断确认持续负载会触发
软件功率限制，但未逐条证明每次离群的原因，不宣称所有记录稳定性达标。

### Conversion-only

| 实现 | W转换 | A转换 | 转换合计 |
|---|---:|---:|---:|
| O0 | 0.063089 | 0.049137 | 0.112276 |
| O1 | 0.062838 | — | 0.062838 |
| O3 store2 | 0.057682 | 0.041034 | 0.098796 |
...
这一独立四模式运行中的O3/O1配对加速比为2.00536；主性能判据仍是前节
24样本/3轮的2.02613及其置信区间，不能用不同运行的绝对延迟交叉相除。

### Cold端到端

| 实现 | W转换（独立摊销） | A转换（独立摊销） | GEMM阶段 | 直接测量total |
|---|---:|---:|---:|---:|
| O0 | 0.062966 | 0.049055 | 0.720128 | 0.862208 |
| O1 | 0.062843 | — | 1.152512 | 1.231872 |
| O3 store2 | 0.057695 | 0.040901 | 0.560128 | 0.687104 |

O3相对O1的配对cold total加速比为1.79687，不是2倍。

### Steady-state端到端

| 实现 | A转换（独立摊销） | GEMM阶段 | 直接测量total |
|---|---:|---:|---:|
| O0 | 0.048960 | 0.774656 | 0.837120 |
| O1 | — | 1.169408 | 1.175552 |
| O3 store2 | 0.040847 | 0.583168 | 0.638464 |

O3相对O1的配对steady total加速比为1.83934。转换为独立批量摊销计时，total为
单次完整路径CUDA Event直接计时；它们的状态与统计口径不同，表中阶段不能简单相加。
所有模式的O3 MSE median/mean仍为0.006653010285119311/0.00757884701115429。
四模式归档SHA-256：`e6757fda88ce3eacbf326525bca053209cb0d91c17887efa019bba04d9026106`。
...
逐样本配对比值范围为[1.987522, 2.081712]，21/24样本超过2倍，因此达标指总体
配对中位数及其置信区间，不是每个样本都超过2倍。72个O3单轮GEMM记录的CV
median/max为3.3274%/6.4539%，47条>=3%；这一口径不同于跨轮合并CV，均保留。
...
选定快速路径的资源为REG=128、STACK=40、LOCAL=0，SASS有5处LDL和5处STL；
LOCAL=0不代表没有spill，必须结合stack与实际指令判断。这些是静态指令位置数量，
不是每次GEMM只读写5次。原始审计同时保留64处IMMA、64处I2F、64处FFMA、
24处LDSM、16处LDGSTS和16处STG.64，不能直接用静态计数替代动态NCU计数。

正式入口的补充memcheck/racecheck均完成140项检查，分别为0 errors和0 hazards，
包括优化路径及形状fallback；所选同一GPU机器码的4096³检查另见v24日志。
production配对归档已下载并校验，SHA-256为
`7490174931b60af01a518a9fa187f35e6edc1ab4df657559d9c5e5ac04162476`。
### v25 production四模式最终复验

`runs/a100_o3_production_four_modes_v25`包含24样本×4实现×4模式=384条唯一记录，
每阶段200个原始计时完整、有限且为正。二进制、CUDA源码和数据manifest均与v25主配对运行一致。
所有O3输出仍逐位等于旧O3，所有模式MSE不变。222个CV>=3%的stage条目全部保留。
以下单位均为ms，数值为24个样本各自中位数的中位数；候选v24数据不混入此表。

#### Conversion-only

| 实现 | W转换 | A转换 | Conversion total |
|---|---:|---:|---:|
| O0 | 0.063058 | 0.049083 | 0.112246 |
| O1 | 0.062838 | — | 0.062838 |
| O3 production | 0.057597 | 0.040960 | 0.098580 |

#### Compute-only

| 实现 | GEMM | 直接total |
|---|---:|---:|
| O0 | 0.739328 | 0.739328 |
| O1 | 1.143296 | 1.149440 |
| O3 production | 0.573440 | 0.579584 |

此独立四模式运行中O3/O1配对GEMM加速为**1.998212×**，略低于2。
不能隐去该结果或把它四舍五入后宣称严格超过2。预先约定的24样本/3轮主配对
门槛由前节2.021739×及CI下界2.012658通过；二者共同支持“约2倍、余量很小”，
而不是无条件稳定超过2倍。四模式包含转换批量测量，运行顺序/缓存/温度状态与
专门的compute-only配对不同，但未逐一隔离这些因素，不能指定其中某个因素为确切原因。

#### Cold端到端

| 实现 | W转换（独立摊销） | A转换（独立摊销） | GEMM阶段 | 直接total |
|---|---:|---:|---:|---:|
| O0 | 0.062743 | 0.048993 | 0.721408 | 0.863744 |
| O1 | 0.062843 | — | 1.149184 | 1.228032 |
| O3 production | 0.057610 | 0.040837 | 0.560128 | 0.687104 |

O3/O1配对cold total加速为1.793877×。

#### Steady-state端到端

| 实现 | A转换（独立摊销） | GEMM阶段 | 直接total |
|---|---:|---:|---:|
| O0 | 0.048947 | 0.774656 | 0.837120 |
| O1 | — | 1.158656 | 1.164800 |
| O3 production | 0.040824 | 0.583168 | 0.638464 |

O3/O1配对steady total加速为1.828420×。转换计时为独立批量摊销，total为单次路径
直接CUDA Event计时，不能简单相加。MSE median/mean分别为：
O1 `9.82338825329489e-9 / 1.1142038139865581e-8`；
O3 `0.006653010285119311 / 0.00757884701115429`。

归档SHA-256：`19a7ee8c2d26d279d079925e080ddce14d0b4f99f1d60e87150d6373884fbd22`。
目标一按主性能口径、数值/ISA/安全/四模式要求完成；不等于O3在所有形状、
运行顺序或端到端口径都比O1快2倍。目标二单独进行，保留SM120旧实现作精确对照。
