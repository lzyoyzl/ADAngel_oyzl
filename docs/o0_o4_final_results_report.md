...
> 更新提示（2026-09-04）：本文表中的 O3 绝对性能对应上一版 production。
> 当前 O3 已晋升为 `m64_n32_k128_aligned_factor_16w`；最新24样本配对性能、MSE、
> 4096³ 四种计时和 SM120 SASS 限制见
> [O3 达到 O1 一半吞吐目标的优化结论](o3_half_o1_optimization_report.md)。
> O0/O1/O2/O4 数据未改变。
...
该表是 `runs/rtx5090_o0_o4_k512` 的同条件横向快照，其中 O3行是优化前实现，不能代表
当前 production。当前 O3独立24样本 compute-only median/mean 为
`2.015720/2.014056 ms`，MSE median/mean 为
`0.006653010195/0.007578844400`；由于不是与本表其他 variant 同一时段的配对运行，
不直接替换表内 O3行。

最终同步后新增的同一时段全量 run `runs/final_b10ed0b_o0_o4` 包含480条有效记录且
全部满足 `CV<3%`。其 O0/O1/O2/O3/O4 compute-only跨样本 median 分别为
`0.767544/0.623776/0.138512/2.065672/7.650752 ms`；O3配对等效吞吐为 O1的
`30.526%`。该run验证当前 O3实现，但本节原表仍保留原始实验快照，避免静默改写既有
报告的 provenance。
