...
enum class O1Implementation {
  ...
  kRegister128x32K64ScaleSharedRowDedup,
...
#pragma unroll
  ...
  auto tCrAccumulator = cute::make_fragment_like<float>(tCrPartial);
  auto tCrRowScale = cute::make_fragment_like<float>(tCrPartial);
  float unique_row_scales[2] = {0.0f, 0.0f};
  cute::clear(tCrAccumulator);
  if constexpr (kDeduplicateRowScale) {
#pragma unroll
    for (int row_slot = 0; row_slot < 2; ++row_slot) {
      // The SM80 16x8 row accumulator layout owns two rows per thread.
      // Items 0/1 use row slot 0 and items 2/3 use row slot 1; the
      // pattern repeats every four items for larger N replication tiles.
      const int representative_item = row_slot * 2;
      const int local_row = cute::get<0>(tCcC(representative_item));
      const int global_row = tile_row + local_row;
      unique_row_scales[row_slot] =
          global_row < m ? a_scale[global_row] : 0.0f;
    }
  } else {
#pragma unroll
    for (int item = 0; item < cute::size(tCrRowScale); ++item) {
...
#pragma unroll
      for (int round = 0; round < Config::kTileN / 32; ++round) {
        ...
        if constexpr (kSparseColumnScaleLoads) {
          // The 128x64 candidate uses an 8x2 warp layout.  A warp in N
          // partition 0 consumes source lanes 0-7/16-23; partition 1
          // consumes lanes 8-15/24-31.  Loading only those lanes halves
          // consumer scale traffic without changing the shuffle mapping.
          constexpr int kWarpRows = Config::kTileM / 16;
          const int consumer_warp = compute_thread >> 5;
          const int warp_column_partition = consumer_warp / kWarpRows;
          const bool owns_scale_lane =
              ((lane >> 3) & 1) == warp_column_partition;
          warp_column_scales[round] = owns_scale_lane
              ? shared_storage.column_scale_factor[
...
#pragma unroll
        ...
        if constexpr (kDeduplicateRowScale) {
          row_scale = unique_row_scales[(item >> 1) & 1];
        } else {
          row_scale = tCrRowScale(item);
        }
...
template <class TmaA, class TmaB>
__global__ __launch_bounds__(O1Register128x32K64Config::kThreadsPerBlock)
void adangel_o1_register_partial_128x32_k64_scale_shared_row_dedup(
    CUTE_GRID_CONSTANT TmaA const tma_a,
    CUTE_GRID_CONSTANT TmaB const tma_b,
    const float* a_scale,
    const uint8_t* w_scale,
...
template <class TmaA, class TmaB>
__global__ __launch_bounds__(O1Register128x64K64Config::kThreadsPerBlock)
void adangel_o1_register_partial_128x64_k64_scale_shared_row_dedup(
    CUTE_GRID_CONSTANT TmaA const tma_a,
    CUTE_GRID_CONSTANT TmaB const tma_b,
    const float* a_scale,
    const uint8_t* w_scale,
  ...
  if constexpr (std::is_same_v<Config, O1Register64K64Config>) {
    ...
    if constexpr (kSparseColumnScaleLoads) {
      adangel_o1_register_partial_128x64_k64_scale_shared_row_dedup_sparse_scale<<<
          ...
          k,
          groups);
    } else if constexpr (kDeduplicateRowScale) {
...
O1Implementation parse_o1_implementation(const std::string& implementation) {
  ...
  if (selected == "register_128x32_k64_scale_shared_row_dedup") {
    return O1Implementation::kRegister128x32K64ScaleSharedRowDedup;
  }
...
py::dict o1_metadata(O1Implementation implementation, int groups) {
  ...
  if (implementation == O1Implementation::kSharedPartial) {
    ...
    const bool row_scale_dedup_128x32 = implementation ==
        O1Implementation::kRegister128x32K64ScaleSharedRowDedup;
    const bool row_scale_dedup_128x64 = implementation ==
            O1Implementation::kRegister128x64K64ScaleSharedRowDedup ||
        implementation ==
            O1Implementation::kRegister128x64K64ScaleSharedRowDedupSparseScale;
    const bool sparse_column_scale_loads = implementation ==
        O1Implementation::kRegister128x64K64ScaleSharedRowDedupSparseScale;
    const bool row_scale_dedup =
        row_scale_dedup_128x32 || row_scale_dedup_128x64;
    ...
    result["implementation_key"] = row_scale_dedup_128x32
    ...
    result["kernel_symbol"] = row_scale_dedup_128x32
        ? "adangel_o1_register_partial_128x32_k64_scale_shared_row_dedup"
        : (sparse_column_scale_loads
        ? "adangel_o1_register_partial_128x64_k64_scale_shared_row_dedup_sparse_scale"
        : (row_scale_dedup_128x64
    ...
    result["row_scale_load_scope"] = row_scale_dedup
        ? "thread_once_per_unique_row"
        : "fragment_item_once";
    if (row_scale_dedup) {
      result["row_scale_loads_per_thread"] = 2;
    } else {
      result["row_scale_loads_per_thread"] = py::none();
    }
